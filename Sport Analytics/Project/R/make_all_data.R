...
# Merge all the data
all_season_data <- rbind(data1,data2,data3,data4,data5,
...
game_shots <- all_season_data[which(all_season_data$name %in% c("shot","goal")),]
write.csv(game_shots, file = "all_season_data.csv", row.names = FALSE)

all_season_data$gameReferenceId <- as.character(all_season_data$gameReferenceId)
all_season_data$gameReferenceId[is.na(all_season_data$gameReferenceId)] <- "?"

# Remove all the unnecessary dataset (to speed up R)
rm(list = ls()[-1])

# Select the shots only
game_shots <- all_season_data[which(all_season_data$name %in% c("shot")),]


# Who was the goalkeeper? Let's add his full name
# Get the (unique) name of the players and their referenceID
library(dplyr)
reference_players <- na.omit(all_season_data[,40:42] %>% distinct())
# Keep only the goalies
reference_goalies <- reference_players[
  reference_players$playerReferenceId %in% game_shots$teamGoalieOnIceRef, ]
colnames(reference_goalies) <- 
  c("goalieFirstName", "goalieLastName", "opposingTeamGoalieOnIceRef")
# Join the dataset using the "opposingTeamGoalieOnIceRef" as key
game_shots <- game_shots %>% 
  left_join(reference_goalies, by = "opposingTeamGoalieOnIceRef")

# Rechange name and call game_shot "all_season_data" again
all_season_data <- game_shots
